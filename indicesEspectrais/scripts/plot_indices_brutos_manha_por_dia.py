...
def gerar(nome: str, config: dict, base: pd.DataFrame) -> None:
    ...
    resumo.to_csv(ROOT / "resultados" / f"{nome}_bruto_manha_por_dia_resumo.csv", index=False,
                  float_format="%.6f")
    ...
    fig.savefig(ROOT / "resultados" / f"{nome}_bruto_manha_por_dia_genotipos.png", dpi=300,
                bbox_inches="tight")
    fig.savefig(ROOT / "resultados" / f"{nome}_bruto_manha_por_dia_genotipos.pdf", bbox_inches="tight")
    plt.close(fig)
