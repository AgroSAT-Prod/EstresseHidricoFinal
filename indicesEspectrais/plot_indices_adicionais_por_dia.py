...
def gerar_indice(nome: str, config: dict, dados_originais: pd.DataFrame) -> None:
    ...
    resumo.to_csv(ROOT / f"{nome}_por_dia_resumo.csv", index=False, float_format="%.6f")
    ...
    fig.savefig(ROOT / f"{nome}_por_dia_genotipos.png", dpi=300, bbox_inches="tight")
    fig.savefig(ROOT / f"{nome}_por_dia_genotipos.pdf", bbox_inches="tight")
    plt.close(fig)
