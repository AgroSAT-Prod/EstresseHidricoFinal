#!/usr/bin/env python3
"""CHLTotal observado × predito em 25/02, distinguindo condição hídrica."""
...
ARQUIVO = RESULTADOS / "25_02_predicoes_cv.csv"
OBSERVADO = "CHL TOTAL (25.02)"
...
def main() -> None:
    ...
    ax.set(xlim=(minimo - margem, maximo + margem), ylim=(minimo - margem, maximo + margem),
           xlabel="CHLTotal observada", ylabel="CHLTotal predita (CV por bloco)",
           title="CHLTotal em 25/02: observado × predito por condição hídrica")
    ax.grid(alpha=.22)
    ax.legend(title="Condição", frameon=True, loc="lower right", fontsize=15, title_fontsize=15)
    fig.tight_layout()
    fig.savefig(RESULTADOS / "25_02_chltotal_observado_x_predito_por_condicao.png", dpi=300, bbox_inches="tight")
    fig.savefig(RESULTADOS / "25_02_chltotal_observado_x_predito_por_condicao.pdf", bbox_inches="tight")
    plt.close(fig)
    pd.DataFrame(linhas).to_csv(RESULTADOS / "25_02_metricas_por_condicao.csv", sep=";", index=False, float_format="%.6f")
